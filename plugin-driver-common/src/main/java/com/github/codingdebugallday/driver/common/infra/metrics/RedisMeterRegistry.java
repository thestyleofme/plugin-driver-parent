...
import com.github.codingdebugallday.driver.common.infra.utils.IpUtil;
...
public class RedisMeterRegistry extends StepMeterRegistry {
    ...
    public RedisMeterRegistry(PluginDatasource pluginDatasource) {
    ...
    private void doPublish(List<Metric> metrics) {
        if (!isClosed()) {
            String key = String.format(KEY_FMT, pluginDatasource.getDatasourceCode(), IpUtil.LOCAL_IP + "-" + getPort());
