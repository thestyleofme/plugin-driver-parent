...
import com.github.thestyleofme.driver.hive2.session.meta.Hive2TableExtra;
 ...
 */
@Slf4j
public class Hive2DriverSession extends AbstractRdbmsDriverSession {
     ...
     * 匹配S3路径
     */
    private static final Pattern S3_P = compile("'s3://(.*?)'");

    public Hive2DriverSession(DataSource dataSource) {
        super(dataSource);
    }
    ...
    public List<Column> columnMetaData(String schema, String tableName) {
        ...
        try (Connection connection = this.dataSource.getConnection();
            ...
            if (null != rs) {
                while (rs.next()) {
                    Hive2Column column = new Hive2Column(rs);
    ...
    public Table tableMetaExtra(String schema, String tableName) {
        Hive2TableExtra tableExtra = new Hive2TableExtra();
        // basic info
    ...
    public SqlGenerator getSqlGenerator() {
        return Hive2SqlGenerator.getInstance();
    }
    ...
    private void parseResult(Map<String, Object> result, String builder) {
        ...
        Matcher ma3 = S3_P.matcher(builder);
        ...
        while (ma3.find()) {
            String path = org.apache.commons.lang3.StringUtils.isEmpty(ma3.group(1)) ? "" : ma3.group(1);
            String[] paths = path.split("/");
            result.put("hdfsUrl", String.format("s3://%s", paths[0]));
            result.put("warehouse", path.substring(paths[0].length()));
        }
    }
