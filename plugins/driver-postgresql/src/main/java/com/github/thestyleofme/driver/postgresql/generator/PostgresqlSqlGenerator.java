package com.github.thestyleofme.driver.postgresql.generator;
...
public class PostgresqlSqlGenerator extends AbstractRdbmsSqlGenerator {
    ...
    private static final String BIG_AUTO_INCREMENT_TYPE = "bigserial";
    ...
    private PostgresqlSqlGenerator() {
    }
    ...
    public String convertAutoincrement(Table table, Column column) {
        if (YES.equals(column.getIsAutoincrement())) {
            if (Types.BIGINT == column.getDataType()) {
                return BIG_AUTO_INCREMENT_TYPE;
            }
    ...
    public String createTable(Table table) {
        ...
        for (Column column : columnList) {
            String sqlFormat = String.format(COLUMN_FT
                    ...
                    // 2. 根据自增判断, 字段类型
                    , Strings.EMPTY.equals(this.convertAutoincrement(table, column)) ? this.convertType(column) : this.convertAutoincrement(table, column)
                    // 3. 是否为NULL
    ...
    public String addPrimaryKey(String schemaName, String tableName, PrimaryKeyBeautify pkBeautify) {
        ...
        return String.format(ADD_PK_FT, this.buildTableName(schemaName, tableName), tableName + "_pk", pkSql);
    }
