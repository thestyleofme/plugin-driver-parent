...
public class MysqlSqlGenerator implements SqlGenerator {
    ...
    public String generateCreateSql(Table table) {
        ...
        sql.append(RIGHT_BRACKET)
                .append(String.format(COMMENT_FORMAT,table.getRemarks()))
        ...
        switch (dataType) {
            ...
            default:
                throw new UnsupportedOperationException(
                        String.format("Mysql generator unsupported data type:[%s],column:[%s]",
    ...
    public String buildPkSql(Table table) {
        ...
        // 单主键｜单组合主键
        String pkSql = pkMap.values().stream()
                .sorted(Comparator.comparingInt(PrimaryKey::getKeySeq))
                .map(PrimaryKey::getColumnName)
                .collect(Collectors.joining(COMMA, BACKQUOTE, BACKQUOTE));
    ...
    public String buildIkSql(Table table) {
        ...
        ikList.stream()
                ...
                .forEach((ikName, ikColumnList) -> {
                    String ikColumns = ikColumnList.stream()
                            .map(IndexKey::getColumnName)
                            .map(this::addBackQuote)
                            .collect(Collectors.joining(COMMA));
                    sql.append(String.format(INDEX_FORMAT
                            , table.getTableSchema()
                            , table.getTableName()
                            , ikColumnList.get(0).getNonUnique() ? "INDEX" : "UNIQUE"
                            , ikName
                            , ikColumns))
                            .append(NEWLINE);
                });
    ...
    public String buildFkSql(Table table) {
        ...
        fkMap.values().stream()
                .collect(Collectors.groupingBy(ForeignKey::getFkName))
                .forEach((fkName, fkList) -> {
                    List<ForeignKey> keyList = fkList.stream()
                            .sorted(Comparator.comparingInt(ForeignKey::getKeySeq))
                            .collect(Collectors.toList());
                    String pkSchema = fkList.get(0).getPkTableSchema();
                    String pkTable = fkList.get(0).getPkTableName();
                    String pkSql = keyList.stream()
                            .map(ForeignKey::getPkName)
                            .map(this::addBackQuote)
                            .collect(Collectors.joining(COMMA));
                    String fkSql = keyList.stream()
                            .map(ForeignKey::getFkName)
                            .map(this::addBackQuote)
                            .collect(Collectors.joining(COMMA));
                    sql.append(String.format(FK_FORMAT,
                            table.getTableSchema(),
                            table.getTableName(),
                            fkSql,
                            pkSchema,
    ...
    public String convertColumnDef(String columnDef) {
        if (StringUtils.isNotEmpty(columnDef)
                && !NULL.equalsIgnoreCase(columnDef)
                // 排除GP的序列默认值
