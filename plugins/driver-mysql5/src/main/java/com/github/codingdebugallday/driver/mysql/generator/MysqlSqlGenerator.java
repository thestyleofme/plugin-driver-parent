package com.github.codingdebugallday.driver.mysql.generator;
...
public class MysqlSqlGenerator extends AbstractRdbmsSqlGenerator {
    ...
    public String createTable(Table table) {
        ...
        List<PrimaryKey> primaryKeys = new ArrayList<>(table.getPkMap().values());
        List<PrimaryKey> addPrimaryKeyList = primaryKeys.stream()
                .filter(pk -> !autoColumn.contains(pk.getColumnName()))
                .collect(Collectors.toList());
        // 主键
        sql.append(this.addPrimaryKey(table.getTableSchema(), table.getTableName(),
                addPrimaryKeyList)).append(NEWLINE);
        // 索引
