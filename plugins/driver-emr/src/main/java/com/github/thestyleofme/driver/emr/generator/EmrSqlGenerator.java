package com.github.thestyleofme.driver.emr.generator;
...
import com.github.thestyleofme.driver.emr.meta.EmrTable;
 ...
 * 生成Emr建表语句
 *
 * @author terry 2020/8/5 15:31
 *
 * </p>
 * @since 1.0
 */
public class EmrSqlGenerator extends AbstractRdbmsSqlGenerator {
    ...
    private static final String CREATE_EXTERNAL_TABLE = "CREATE EXTERNAL TABLE if not exists %s.%s";
    ...
    private static final String S3_LOCATION_FT = "location 's3://%s/warehouse/%s.db/%s'";
    ...
    private static class Hive2SqlGeneratorHolder {
        private static final SqlGenerator INSTANCE = new EmrSqlGenerator();
    }
    ...
    public String createTable(Table table) {
        ...
        sql.append(String.format(CREATE_EXTERNAL_TABLE, schema, tableName)).append(LEFT_BRACKET).append(NEWLINE);
       ...
       /* String rowFormat = tmp.getRowFormat();
        ...
        sql.append(NEWLINE).append(String.format(ROW_FORMAT, APO_STROPHE + COMMA + APO_STROPHE)).append(NEWLINE);
        // 是否指定了存储格式
   /*     String stored = tmp.getStored();
        ...
        // S3指定location
        sql.append(SPACE).append(String.format(S3_LOCATION_FT, this.getBucketName(table), table.getTableSchema(), tableName));
    ...
    private String getBucketName(Table table) {
        return Optional.ofNullable((String) table.getExtra().get(BUCKET_NAME)).orElse(table.getTableSchema());
    }
        ...
        switch (dataType) {
            ...
            default:
                throw new UnsupportedOperationException(
                        String.format("Emr generator unsupported data type:[%s],column:[%s]",
    ...
    public String dropColumn(Column column) {
        EmrColumn tmp = (EmrColumn) column;
        ...
        allColumns.stream()
                ...
                .forEach(c -> tmpSql.append(String.format("%s %s, %n", c.getColumnName(), this.convertType(c))));
