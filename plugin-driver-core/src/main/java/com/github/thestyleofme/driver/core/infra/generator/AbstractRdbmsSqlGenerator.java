...
import static com.github.thestyleofme.plugin.core.infra.constants.BaseConstant.Symbol.EMPTY;
...
public abstract class AbstractRdbmsSqlGenerator implements SqlGenerator {
    ...
    public String convertAutoincrement(Table table, Column column) {
        if (YES.equals(column.getIsAutoincrement())) {
            if (table.getPrimaryKeyBeautify() != null
                    && table.getPrimaryKeyBeautify().getColumnList().size() == 1
                    && table.getPrimaryKeyBeautify().getColumnList().get(0).getColumnName().equals(column.getColumnName())) {
